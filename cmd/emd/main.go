...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "emd",
		Short:             "eventMaker App Daemon (server)",
	...
	executor := cli.PrepareBaseCmd(rootCmd, "EM", DefaultNodeHome)
...
func appExporter() server.AppExporter {
	return func(logger log.Logger, db dbm.DB, _ io.Writer, _ int64, _ bool, _ []string) (
		...
		dapp := app.NewEventMarketApp(logger, db)
...
func InitCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			fmt.Printf("Initialized emd configuration and bootstrapping files in %s...\n", viper.GetString(cli.HomeFlag))
			return nil
		},
	}
...
$ emd add-genesis-account cosmos1tse7r2fadvlrrgau3pa0ss7cqh55wrv6y9alwh 1000STAKE,1000eventtoken
`),
